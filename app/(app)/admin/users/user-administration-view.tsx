...
export function UserAdministrationView({ result }: { result: UserAdministrationResult }) {
  ...
  return <div className="space-y-6"><header><h1 className="text-3xl font-bold">Benutzer &amp; Rollen</h1><p className="mt-2">Hier siehst du die vorhandenen Benutzer und ihre aktuellen Anwendungsrollen.</p><p className="mt-2 rounded border border-amber-200 bg-amber-50 p-3 text-sm">Rollenänderungen und Einladungen werden in einem späteren Schritt ergänzt.</p></header><Card>{users.length === 0 ? <p>Keine Benutzer gefunden.</p> : <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b"><th className="p-2">E-Mail</th><th className="p-2">Rolle</th><th className="p-2">Profilstatus</th><th className="p-2">Auth-Status</th><th className="p-2">Erstellt am</th></tr></thead><tbody>{users.map((user) => <tr className="border-b last:border-0" key={user.user_id}><td className="p-2 font-medium">{user.email ?? "Nicht verfügbar"} {user.is_current_user ? <Badge tone="ok">Du</Badge> : null}</td><td className="p-2">{user.role ? roleLabels[user.role] : "Keine gültige Rolle"}</td><td className="p-2"><Badge tone={user.profile_status === "active" ? "ok" : "warn"}>{profileLabels[user.profile_status]}</Badge></td><td className="p-2">Nicht eindeutig bestimmbar</td><td className="p-2">{formatDate(user.created_at)}</td></tr>)}</tbody></table></div>}</Card><nav aria-label="Seitennavigation" className="flex items-center justify-between"><span>{page > 1 ? <Link className="rounded border px-3 py-2" href={`/admin/users?page=${page - 1}`}>Zurück</Link> : <span aria-disabled="true" className="rounded border px-3 py-2 text-slate-400">Zurück</span>}</span><span>Seite {page}</span><span>{hasNextPage ? <Link className="rounded border px-3 py-2" href={`/admin/users?page=${page + 1}`}>Weiter</Link> : <span aria-disabled="true" className="rounded border px-3 py-2 text-slate-400">Weiter</span>}</span></nav></div>;
}
