...
export default async function ProjectDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ created?: string; updated?: string; review_updated?: string }> }) {
  ...
  const mayReviewProject = parsedRole.success && canChangeProjectStatus(parsedRole.data) && canChangeProjectClass(parsedRole.data) && canChangeHumanReview(parsedRole.data);
  const typedProjectStatus = project.status as ProjectStatus;
  const typedProjectClass = project.project_class as ProjectClass | null;
  const allowedTargetStatuses = getAllowedProjectStatusTransitions(typedProjectStatus);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <dl className="grid gap-4 md:grid-cols-2">
          ...
          <div><dt className="font-medium">Status</dt><dd><Badge tone={project.requires_human_review ? "warn" : "default"}>{statusToLabel(typedProjectStatus)}</Badge></dd></div>
          <div><dt className="font-medium">Projektklasse</dt><dd>{projectClassDisplay(typedProjectClass)}</dd></div>
      ...
      <Card className="space-y-4">
        <div>
          ...
          <p className="text-sm text-slate-600">Status, Projektklasse und Human Review werden getrennt von den Projektstammdaten geprüft.</p>
        </div>
        {mayReviewProject ? (
          <ProjectReviewForm project={{ id: project.id, status: typedProjectStatus, project_class: typedProjectClass, requires_human_review: project.requires_human_review }} allowedTargetStatuses={allowedTargetStatuses} />
        ) : (
          <p className="text-slate-600">Sie sind nicht berechtigt, die Projektprüfung zu bearbeiten.</p>
        )}
      </Card>
    </div>
  );
