...
export function Nav({ role }: { role: Role | null }) {
  const canViewAdministration = role !== null && canViewProjectMediaOrphanInventory(role);

  return <nav className="border-b bg-white"><div className="mx-auto flex max-w-6xl items-center gap-5 px-4 py-4"><Link className="font-bold text-teal-800" href="/dashboard">KlimaGuy</Link><Link href="/customers">Kunden</Link><Link href="/projects">Projekte</Link>{canViewAdministration ? <div aria-label="Administration" className="border-l pl-5"><span className="mr-5 font-semibold">Administration</span><Link href="/admin/project-media/orphans">Medien-Inventur</Link></div> : null}<form action="/auth/logout" method="post" className="ml-auto"><button className="text-slate-600">Logout</button></form></div></nav>;
}
