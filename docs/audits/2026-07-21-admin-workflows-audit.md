...
- **Implementierungsstatus:** Implementiert auf Branch `codex/ap-07-project-review-workflow`; der Gesamtstatus dieses Audits wurde nicht auf vollständig freigegeben geändert.
- **Baseline-Commit:** `f2bfcc95a455e1be574053802d1bc07399d133dc`
...
- **Implementierter Projektprüfungsworkflow:** Admins und Reviewer sehen auf der Projektdetailseite den getrennten Bereich `Projektprüfung`, wählen ausschließlich den aktuellen Status oder erlaubte Zielstatus, setzen eine Projektklasse A-D, setzen das Human-Review-Kennzeichen und speichern alle drei Werte gemeinsam. Nach Erfolg erfolgt die Weiterleitung zu `/projects/[id]?review_updated=1` mit Erfolgsmeldung `Projektprüfung wurde aktualisiert.`
- **Rollenprüfung:** Die Update-Action lädt Benutzer und Profil, validiert `roleSchema` und verlangt gemeinsam `canChangeProjectStatus(role)`, `canChangeProjectClass(role)` und `canChangeHumanReview(role)`. Fehlt eine Berechtigung, erfolgt kein Teilupdate.
- **Statusübergangsprüfung:** Der aktuelle Projektstatus wird unmittelbar vor dem Update aus einem aktiven Projekt geladen. Zielstatus werden mit `projectStatusSchema` validiert; unterschiedliche Zielstatus werden mit der zentralen AP-01-Funktion `isProjectStatusTransitionAllowed(currentStatus, targetStatus)` geprüft. Gleicher Status ist zulässig, damit Klasse und Human Review separat aktualisiert werden können.
- **Verwendete AP-01-Domainhelper:** `PROJECT_STATUSES`, `PROJECT_CLASSES`, `projectStatusSchema`, `projectClassSchema`, `requiresHumanReviewSchema`, `getAllowedProjectStatusTransitions`, `isProjectStatusTransitionAllowed`, `statusToLabel`, `projectClassToLabel`, `projectClassToDescription`, `canChangeProjectStatus`, `canChangeProjectClass` und `canChangeHumanReview`.
- **Projektklassenvalidierung:** `updateProjectReviewSchema` verlangt `project_class` über `projectClassSchema`; `null` wird beim Speichern abgewiesen.
- **Human-Review-Validierung:** `requires_human_review` wird explizit aus der Checkbox als Boolean gemappt und mit `requiresHumanReviewSchema` validiert; String-Werte wie `false`, `0` oder `off` werden im Schema nicht akzeptiert.
- **Patch-Allowlist:** Das Update-Payload enthält ausschließlich `status`, `project_class` und `requires_human_review`.
- **Mass-Assignment-Schutz:** FormData wird explizit auf `project_id`, `status`, `project_class` und `requires_human_review` abgebildet; `project_id` dient nur als Filter. Manipulierte Felder wie `customer_id`, `title`, `installation_address`, `postal_code`, `city`, `summary`, `created_by`, `deleted_at`, `role` oder Metadaten gelangen nicht in das Payload.
- **Schutz soft gelöschter Projekte:** Projektlade- und Update-Abfragen filtern `deleted_at IS NULL`; unbekannte oder soft gelöschte Projekte werden nicht als Erfolg behandelt.
- **IDOR-Schutz:** Authentifizierung, Profilprüfung, Rollenvalidierung, UUID-Validierung, aktive Projektladung, konkrete `id`-Filterung, statusbasierter Concurrency-Filter und vorhandene RLS wirken zusammen.
- **Statusbasierter Concurrency-Schutz:** Das Update filtert nach `id`, dem unmittelbar zuvor geladenen `currentStatus` und `deleted_at IS NULL`. Kein betroffener Datensatz liefert `Das Projekt wurde zwischenzeitlich geändert. Bitte laden Sie die Seite neu.`
- **Verhalten bei `closed`:** `closed` besitzt weiterhin keine ausgehenden Statusübergänge. Die UI bietet keine Zielstatus an und sendet den aktuellen Status als unveränderten Wert mit.
- **Fachentscheidung closed:** Projektklasse und Human-Review-Kennzeichen bleiben in AP-07 auch bei Status closed bearbeitbar. Der Status closed selbst besitzt weiterhin keine ausgehenden Statusübergänge.
- **Revalidierungsverhalten:** Nach erfolgreichem Update werden `/projects`, `/projects/[id]` und die serverseitig aus dem aktiven Projekt stammende Kundendetailseite `/customers/[customerId]` revalidiert.
- **Testumfang:** Ergänzt wurden 32 Tests für Schema, zentrale Berechtigungen, Statusübergangs-Serviceintegration, Auth-/Profilfehler, Projekt-ID und aktive Projektladung, statusbasierten Concurrency-Schutz, Payload-Allowlist, neutrale Fehler und FormData-Mapping.
- **Ausgeführte Merge-Gates:** Baseline vor Implementierung: `npm install`, `npm run typecheck`, `npm run lint`, `npm test` und `npm run build` erfolgreich mit 94 Ausgangstests. Nach Implementierung: `npm run typecheck`, `npm run lint`, `npm test`, `npm run build` und `git diff --check` erfolgreich.
- **Audit-Log-Status:** Kein neuer Audit-Log-Schreibmechanismus. `project.status_changed`, `project.class_changed` und `project.human_review_changed` werden nicht in `audit_log` geschrieben, weil weiterhin kein sicherer, dokumentierter und getesteter Mechanismus freigegeben ist.
- **Fehlende Statushistorie:** AP-07 speichert ausschließlich den aktuellen Projektstatus. Eine historische Nachverfolgung von Statuswechseln ist ohne separates Audit-/Historien-Arbeitspaket nicht verfügbar.
- **Bekannte Einschränkungen:** Das erfolgreiche Vercel-Production-Deployment von `main` konnte lokal nicht technisch verifiziert werden. Lokale Branch-Informationen enthielten keinen separaten Branch `main`; AP-07 wurde vom bereitgestellten aktuellen Arbeitsstand mit AP-06 als HEAD abgezweigt. AP-07 enthält keine Projektstammdatenänderung, keine Statushistorie, keine Notizen, keinen Projekt-Soft-Delete und keinen Audit-Schreibpfad.
- **Rollback-Anweisung:** Den AP-07-Commit beziehungsweise den Pull Request vollständig zurücksetzen; es wurden keine Migrationen, RLS-Policies, Trigger, Datenbankfunktionen, Tabellen oder Spalten geändert.
