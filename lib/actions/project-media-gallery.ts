...
export async function getProjectMediaGallery(projectId: string) {
  ...
  return getProjectMediaGalleryWithDataSource({
    ...
    listMedia: async (id) => supabase.from("project_media")
      .select("id,project_id,category,media_type,mime_type,file_size_bytes,caption,created_at,storage_bucket,storage_path,source_conversation_id,source_message_id")
      .eq("project_id", id).eq("upload_status", "ready").eq("physical_state", "present").is("deleted_at", null)
