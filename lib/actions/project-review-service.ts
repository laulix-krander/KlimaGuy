...
type ActiveProjectReviewRow = { id: string; customer_id: string; status: ProjectStatus };
...
export type ActiveProjectReviewQuery = {
  select(columns: "id,customer_id,status"): {
    eq(column: "id", value: string): {
      is(column: "deleted_at", value: null): {
        single(): QueryResult<ActiveProjectReviewRow>;
      };
    };
  };
...
export type UpdateProjectReviewDataSource = {
  auth: AuthQuery;
  from(table: "profiles"): ProjectReviewProfilesQuery;
  from(table: "projects"): ActiveProjectReviewQuery;
};
...
export function formDataToUpdateProjectReviewInput(formData: FormData): { projectId: unknown; values: unknown } {
  return {
    ...
    values: {
      ...
      requires_human_review: formData.get("requires_human_review") === "on" || formData.get("requires_human_review") === "true",
    },
  };
}
...
export async function updateProjectReviewWithDataSource(
  dataSource: UpdateProjectReviewDataSource,
  projectId: unknown,
  input: unknown,
