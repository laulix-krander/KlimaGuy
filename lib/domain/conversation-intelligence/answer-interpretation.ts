...
export function interpretNormalizedAnswer(input: unknown): InterpretationResult {
  ...
  const proposal = (transition_type: StateTransitionProposal["transition_type"], changes: boolean, evidence_proposals: readonly EvidenceProposal[], claim_proposals: readonly KnowledgeClaimProposal[], superseded_claim_ids: readonly string[], retry_outcome: StateTransitionProposal["retry_outcome"], explanation_codes: readonly ExplanationCode[]): InterpretationResult => interpretationResultSchema.parse({ success: true, proposal: { ...base, transition_type, proposed_state_version: context.current_state_version + (changes ? 1 : 0), evidence_proposals, claim_proposals, superseded_claim_ids, retry_outcome, explanation_codes }, ...(transition_type === "duplicate_no_change" ? { code: "idempotent_success" } : {}) }) as InterpretationResult;
  if (answer.outcome === "skipped") return proposal("skip_recorded", false, [], [], [], "skipped", ["mapping_rule_applied", "skip_without_property_claim"]);
  if (rule.status === "deferred") return failure("unsupported_text_mapping", { requires_human_review: true });
  ...
  let value: number | boolean | null; let value_type: "number" | "boolean" | "unknown"; let epistemic_status: "reported" | "unknown" | "assumed"; let approximation: "exact" | "approximate" | undefined; let evidence: readonly EvidenceProposal[] = [customerEvidence];
  ...
  if (answer.outcome === "unknown") { value = null; value_type = "unknown"; epistemic_status = "unknown"; transition = "unknown_recorded"; explanations.push("unknown_value_recorded"); }
    ...
    if (answeredValue.kind === "number_range") return failure("numeric_range_not_supported", { requires_human_review: true });
    if (answeredValue.kind === "text") return failure("unsupported_text_mapping", { requires_human_review: true });
