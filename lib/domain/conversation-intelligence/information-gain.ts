...
import { PLANNER_EVIDENCE_CONTEXT_TARGET_KEYS, plannerEvidenceContextSchema } from "./planner-evidence-context";
...
export const INFORMATION_GAIN_REASON_CODES=["new_information_expected","alternate_question_strategy","dependency_context_changed","same_context_as_previous_attempt","existing_descriptive_evidence_context","customer_path_exhausted","additional_evidence_required","retry_limit_reached","future_photo_path","site_check_path","no_available_collection_path"] as const;
...
export type InformationGainAssessmentInput=Readonly<z.input<typeof informationGainAssessmentInputSchema>>;
...
export function assessInformationGain(input:InformationGainAssessmentInput):InformationGainAssessment{
 ...
 const evidencePathCovered=Boolean(value.evidence_target_key&&value.evidence_context.some(item=>item.covered_target_keys.includes(value.evidence_target_key!)));
 ...
 if(exhaustedStatuses.has(value.collection_status)||(value.collection_status==="asked"&&["leave_information_open","customer_does_not_know","requires_additional_evidence"].includes(value.last_answer_meaning??""))){
  ...
  if(value.available_evidence_channels.future_photo_request&&!evidencePathCovered)return result("additional_evidence_needed","future_photo_request",false,["additional_evidence_required","future_photo_path"]);
  ...
  return result("customer_path_exhausted","leave_open",false,["customer_path_exhausted",...(evidencePathCovered?["existing_descriptive_evidence_context" as const]:["same_context_as_previous_attempt" as const]),"no_available_collection_path"]);
 }
