...
export const observationClaimMappingRuleSchema=z.object({
  ...
  output_strength:z.literal("descriptive_fact").nullable(),output_value:z.literal(true).nullable(),
...
type RuleSeed=Readonly<{target_key:ObservationClaimMappingRule["target_key"];observation_type:ObservationClaimMappingRule["observation_type"];review_class:ObservationClaimMappingRule["review_class"];reason:string;entity_type?:EntityType;property_key?:typeof ALL_PROPERTY_KEYS[number];quality?:EvidenceQuality;actors?:readonly ("admin"|"reviewer"|"ai")[]}>;
const seeds:readonly RuleSeed[]=[
  {target_key:"room_overview",observation_type:"room_overview_visible",review_class:"auto_proposable",reason:"claim_proposal_created",entity_type:"room",property_key:"room_overview_context_observed"},{target_key:"room_overview",observation_type:"window_visible",review_class:"auto_observable",reason:"descriptive_observation_only",entity_type:"room"},{target_key:"room_overview",observation_type:"door_visible",review_class:"auto_observable",reason:"descriptive_observation_only",entity_type:"room"},{target_key:"room_overview",observation_type:"wall_area_visible",review_class:"auto_observable",reason:"no_lossless_knowledge_property",entity_type:"room"},{target_key:"room_overview",observation_type:"measurement_reference_visible",review_class:"auto_observable",reason:"no_lossless_knowledge_property",entity_type:"room"},
  {target_key:"indoor_area_overview",observation_type:"possible_indoor_mounting_area_visible",review_class:"human_review_required",reason:"placement_not_determined",entity_type:"room"},{target_key:"outdoor_area_overview",observation_type:"possible_outdoor_mounting_area_visible",review_class:"human_review_required",reason:"placement_not_determined"},{target_key:"line_route_context",observation_type:"line_route_context_visible",review_class:"auto_proposable",reason:"claim_proposal_created",property_key:"line_route_context_observed"},{target_key:"line_route_context",observation_type:"wall_penetration_context_visible",review_class:"auto_proposable",reason:"claim_proposal_created",property_key:"wall_penetration_context_observed"},{target_key:"electrical_area",observation_type:"electrical_connection_visible",review_class:"site_check_only",reason:"electrical_suitability_site_check_only"},{target_key:"accessibility_context",observation_type:"accessibility_context_visible",review_class:"auto_observable",reason:"safe_access_not_observed"},
  {target_key:"indoor_area_overview",observation_type:"indoor_area_visible",review_class:"auto_proposable",reason:"claim_proposal_created",entity_type:"room",property_key:"indoor_installation_area_observed"},{target_key:"outdoor_area_overview",observation_type:"outdoor_area_visible",review_class:"auto_proposable",reason:"claim_proposal_created",property_key:"outdoor_installation_area_observed"},{target_key:"outdoor_area_overview",observation_type:"accessibility_context_visible",review_class:"auto_observable",reason:"safe_access_not_observed"},
];
const rules=seeds.map(seed=>observationClaimMappingRuleSchema.parse({rule_key:`v1:${seed.target_key}:${seed.observation_type}:visible:${seed.entity_type??"installation"}:${seed.property_key??"none"}`,version:1,target_key:seed.target_key,observation_type:seed.observation_type,observation_value:"visible",entity_type:seed.entity_type??"installation",property_key:seed.property_key??null,review_class:seed.review_class,permitted_property:seed.property_key??null,permitted_epistemic_status:seed.property_key?"observed":null,minimum_evidence_quality:seed.quality??"sufficient_for_observation",allowed_source_actors:seed.actors??["admin","reviewer","ai"],reason_codes:[seed.reason],site_check_boundary:seed.review_class==="site_check_only",output_strength:seed.property_key?"descriptive_fact":null,output_value:seed.property_key?true:null}));
...
export function proposeKnowledgeClaimFromObservation(input:unknown):ObservationClaimMappingResult{
  ...
  const propertyKey=rule.property_key!;
  const strengthValidation=validateClaimStrengthForProperty({property_key:propertyKey,strength:"descriptive_fact",epistemic_status:"observed",actor_class:first.source_actor_class});
  if(!strengthValidation.success)return noClaim("human_review_required",ids,["actor_not_permitted"],rule.review_class,true);
  const effective=getEffectiveClaims(value.knowledge_state).filter(claim=>claim.entity_type===value.entity_type&&claim.entity_id===value.entity_id&&claim.property_key===propertyKey);
  if(effective.some(claim=>claim.value_type==="boolean"&&claim.value===true&&claim.epistemic_status==="observed"))return noClaim("observation_only",ids,["duplicate_no_change"],rule.review_class);
  if(effective.length)return noClaim("human_review_required",ids,[effective.some(claim=>claim.evidence.some(evidence=>evidence.actor_class==="reviewer"||evidence.evidence_status==="manually_corrected"))?"reviewer_protected_claim":"customer_claim_conflict"],rule.review_class,true);
  const sourceType=first.source_actor_class==="ai"?"ai_analysis":first.source_actor_class==="reviewer"?"reviewer_correction":"project_media";
  const evidence={evidence_id:value.proposal_ids.evidence_id,source_type:sourceType,source_id:first.evidence_id,actor_class:first.source_actor_class,observed_at:first.observed_at,evidence_status:"active" as const};
  const claim=knowledgeClaimProposalSchema.parse({claim_id:value.proposal_ids.claim_id,project_id:value.project_id,entity_type:value.entity_type,entity_id:value.entity_id,property_key:propertyKey,value:true,value_type:"boolean",epistemic_status:"observed",knowledge_strength:"descriptive_fact",evidence:[evidence],based_on_state_version:value.knowledge_state.state_version,proposed_state_version:value.knowledge_state.state_version+1,proposal_reason_codes:[]});
  return freeze(observationClaimMappingResultSchema.parse({kind:"claim_proposal",mapping_rule_version:1,observation_ids:ids,reason_codes:["claim_proposal_created"],review_class:"auto_proposable",requires_human_review:false,causes_state_change:false,application_status:"not_applied",claim_proposal:claim,evidence_proposals:[evidence]}));
}
