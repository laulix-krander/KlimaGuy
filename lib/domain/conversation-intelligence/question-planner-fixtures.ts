...
export function createSyntheticPlannerContext(state: KnowledgeState = SYNTHETIC_SINGLE_ROOM_STATES.A, overrides: Partial<PlannerContext> = {}): PlannerContext {
  ...
  return { project_id: state.project_id, conversation_id: state.conversation_id, state_version: state.state_version, knowledge_state: state, information_collection_state:{project_id:state.project_id,conversation_id:state.conversation_id,version:0,items:[],updated_at:AT}, intermediate_assessment: assessment.data, missing_information: deriveMissingInformation(state), target_readiness_level: "level_3_preliminary_installation", retry_state: [],revisit_triggers:[], customer_effort_state: { consecutive_technical_questions: 0, unanswered_questions: 0, repeated_questions: 0 }, created_at: AT, ...overrides };
}
