...
export const TEMPLATE_SEMANTIC_MODES = ["technical_property", "customer_knowledge", "customer_preference", "customer_observation", "collection_capability"] as const;
...
export type QuestionTemplate = Readonly<{ template_key: string; template_version: number; locale: QuestionTemplateLocale; message_kind: QuestionMessageKind; supported_action_type: PlannerActionType; supported_answer_type?: "text" | "boolean" | "approximate_number"; semantic_mode?: typeof TEMPLATE_SEMANTIC_MODES[number]; information_key?: PropertyKey; primary_text: string; supporting_text?: string; help_text?: string; examples: readonly string[]; controlled_parameter_keys: readonly ControlledParameterKey[]; answer_contract?: AnswerContract; retry_variant_of?: string; retry_attempt?: number; customer_visible: boolean; status: typeof QUESTION_TEMPLATE_STATUSES[number]; accessibility_text?: string }>;
export type RenderedCustomerInteraction = Readonly<{ project_id: string; conversation_id: string; decision_id: string; template_key: string; template_version: number; locale: QuestionTemplateLocale; message_kind: QuestionMessageKind; primary_text: string; supporting_text?: string; help_text?: string; examples: readonly string[]; answer_contract?: AnswerContract; answer_options: readonly AnswerOption[]; customer_visible: boolean; accessibility_text?: string }>;
