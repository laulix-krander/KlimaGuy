...
export const MEDIA_DEPENDENCY_TYPES = ["evidence_interpretation", "observation_followup", "claim_proposal_review", "claim_apply", "claim_correction", "offer_preparation", "offer_open", "project_execution"] as const;
...
export const MEDIA_DEPENDENCY_REASON_CODES = [
  ...
  "offer_preparation_open", "offer_open", "project_execution_open",
...
export type MediaDependencyProjectionInput = Readonly<{
  ...
  current_execution?: Source & { status: "not_started" | "active" | "completed" | "cancelled"; accepted_offer_id: string };
...
export type DerivedMediaDependency = Readonly<{
  evidence_id: string; dependency_type: DependencyType; source_record_kind: "interpretation_run" | "observation" | "claim_proposal" | "knowledge_correction" | "project_offer" | "project_execution";
...
export function deriveProjectMediaDependencies(input: MediaDependencyProjectionInput): MediaDependencyProjectionResult {
  ...
  for (const evidence of input.evidence) {
    ...
    if (input.current_execution) {
      const execution = input.current_execution;
      const open = execution.status === "not_started" || execution.status === "active";
      dependencies.push({ evidence_id: evidence.id, dependency_type: "project_execution", source_record_kind: "project_execution", source_record_id: execution.id, source_revision: execution.revision, status: open ? "open" : "resolved", reason_codes: open ? ["project_execution_open"] : [] });
    }
  }
  const offer = input.current_offer;
  const execution = input.current_execution;
  const executionApplicable = offer?.status === "accepted";
  const executionNotApplicable = offer?.status === "rejected" && !execution && (input.project_status === "rejected" || input.project_status === "closed");
  const executionConsistent = !!offer && !!execution && execution.accepted_offer_id === offer.id && offer.status === "accepted" &&
    ((execution.status === "not_started" || execution.status === "active") ? input.project_status === "accepted" : input.project_status === "closed");
  if (execution && !executionConsistent) inconsistent = true;
  const missing: MissingAuthority[] = [];
  if (!offer) missing.push("offer", "execution");
  else if ((executionApplicable && !executionConsistent) || (!executionApplicable && !executionNotApplicable)) missing.push("execution");
