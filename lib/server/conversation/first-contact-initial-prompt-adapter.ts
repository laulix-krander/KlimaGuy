...
import { z } from "zod";
import { initializeFirstContactPrompt, type InitialPromptResult } from "./first-contact-initial-prompt";

/** Productive server-only entry point. It accepts only the persisted conversation identity. */
export async function runFirstContactInitialPrompt(conversationId: string): Promise<InitialPromptResult> {
  ...
  if (!url.success || !key.success) return { status: "persistence_failed", diagnostic: { diagnostic_code: "initial_prompt_adapter_unavailable", stage: "initial_prompt_adapter" } };
