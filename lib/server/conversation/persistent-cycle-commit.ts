...
const rpcFailureSchema = z.object({
  ...
  code: z.enum(["invalid_input", "command_not_found", "command_not_claimed", "stale_runtime_revision", "stale_knowledge_version", "interaction_not_current", "message_conversation_mismatch", "duplicate_conflict", "runtime_invariant_failed", "persistence_failed"]),
}).strict();

export type PersistentCycleCommitRpc = {
  rpc(name: "commit_customer_message_cycle" | "fail_customer_message_cycle" | "complete_customer_message_human_review", args: { target_command_id: string; commit_payload?: unknown; failure_code?: string; review_payload?: unknown }): Promise<{ data: unknown; error: unknown }>;
};

function failed(code: z.infer<typeof rpcFailureSchema>["code"], commandId: string): PersistentCycleResult {
  const mapped = code === "command_not_found" ? "message_not_found"
...
export async function commitCustomerMessageCycle(source: PersistentCycleCommitRpc, input: PersistentCycleCommit): Promise<PersistentCycleResult> {
  ...
  if (controlled.success) return failed(controlled.data.code, identities.data.command_id);
...
export async function failCustomerMessage(source: PersistentCycleCommitRpc, commandId: string, code: z.infer<typeof failureCode>): Promise<void> {
  const input = z.object({ commandId: uuid, code: failureCode }).strict().safeParse({ commandId, code });
  if (!input.success) return;
  await source.rpc("fail_customer_message_cycle", { target_command_id: input.data.commandId, failure_code: input.data.code });
}

/** Separate controlled terminal boundary; no reviewer, approval, or descriptive claim is supplied. */
export async function completeCustomerMessageWithHumanReview(source: PersistentCycleCommitRpc, input: PersistentCycleHumanReview): Promise<PersistentCycleResult> {
  ...
  const result = await source.rpc("complete_customer_message_human_review", { target_command_id: parsed.data.command_id, review_payload: parsed.data });
