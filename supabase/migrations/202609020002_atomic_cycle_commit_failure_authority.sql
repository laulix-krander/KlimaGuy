-- AP-16-06-01E: one service-only transaction for an already calculated customer-message cycle.
alter table public.conversation_cycle_commands add column commit_payload_hash bytea;

create table public.conversation_cycle_events (
 id uuid primary key,
 command_id uuid not null references public.conversation_cycle_commands(id) on delete restrict,
 conversation_id uuid not null references public.conversations(id) on delete restrict,
 project_id uuid not null references public.projects(id) on delete restrict,
 sequence integer not null check(sequence > 0),
 event_type text not null check(event_type in ('customer_answer_interpreted','knowledge_claim_recorded','knowledge_claim_superseded','answer_unknown_recorded','answer_skipped','assumption_confirmed','assumption_rejected','assumption_deferred','human_review_requested','conversation_cycle_completed')),
 actor_class text not null check(actor_class in ('customer','system')),
 ...
 constraint cycle_event_metadata_object check(jsonb_typeof(metadata)='object'),
 constraint cycle_event_metadata_size check(octet_length(metadata::text)<=4096)
);
alter table public.conversation_cycle_events enable row level security;
revoke all on public.conversation_cycle_events from public,anon,authenticated;
grant select on public.conversation_cycle_events to authenticated;
create policy "cycle event staff read" on public.conversation_cycle_events for select to authenticated
 using(auth.uid() is not null and public.current_app_role() in ('admin','reviewer'));
create trigger cycle_events_append_only before update or delete on public.conversation_cycle_events
...
create or replace function public.guard_cycle_command_history() returns trigger language plpgsql set search_path=public,pg_temp as $$ begin
 if old.status in ('completed','stale','human_review_required') or new.id<>old.id or new.conversation_id<>old.conversation_id
  ...
  or (old.commit_payload_hash is not null and new.commit_payload_hash is distinct from old.commit_payload_hash)
 then raise exception 'cycle_command_immutable'; end if; return new;
end $$;

create function public.commit_customer_message_cycle(target_command_id uuid,commit_payload jsonb) returns jsonb
...
begin
 ...
 update public.conversation_cycle_commands set status='completed',result_code=result_kind,result_runtime_revision=next_revision,result_knowledge_version=resulting_version,outbound_message_id=outbound_id,commit_payload_hash=payload_hash,completed_at=statement_timestamp() where id=cmd.id;
...
create function public.fail_customer_message_cycle(target_command_id uuid,failure_code text) returns jsonb
language plpgsql security definer set search_path=public,pg_temp as $$
...
begin
 ...
 update public.conversation_cycle_commands set status='failed',result_code=failure_code,failed_at=statement_timestamp() where id=cmd.id;
 return jsonb_build_object('success',true,'code','failed','command_id',cmd.id);
end $$;

create function public.complete_customer_message_human_review(target_command_id uuid,review_payload jsonb) returns jsonb
language plpgsql security definer set search_path=public,pg_temp as $$
...
begin
 ...
 update public.conversation_cycle_commands set status='human_review_required',result_code='human_review',result_runtime_revision=next_revision,result_knowledge_version=k,completed_at=statement_timestamp() where id=cmd.id;
...
revoke all on function public.commit_customer_message_cycle(uuid,jsonb),public.fail_customer_message_cycle(uuid,text),public.complete_customer_message_human_review(uuid,jsonb) from public,anon,authenticated;
grant execute on function public.commit_customer_message_cycle(uuid,jsonb),public.fail_customer_message_cycle(uuid,text),public.complete_customer_message_human_review(uuid,jsonb) to service_role;
comment on function public.commit_customer_message_cycle(uuid,jsonb) is 'Service-only atomic persistence of an already calculated AP-16 cycle; no planning, rendering, interpretation, delivery, or AI.';
comment on function public.fail_customer_message_cycle(uuid,text) is 'Content-free technical cycle failure authority; call separately only after a rolled-back success commit.';
