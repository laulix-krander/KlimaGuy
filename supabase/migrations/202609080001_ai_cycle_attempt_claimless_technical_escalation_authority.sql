-- AP-16-06-06D-1B: provider-neutral AI attempt, deferral, claimless completion, and technical escalation authority.
alter table public.conversation_cycle_commands
 add column ai_inference_attempt_count integer not null default 0;

alter table public.conversation_cycle_commands
 add constraint conversation_cycle_commands_ai_inference_attempt_count_check
 check (ai_inference_attempt_count between 0 and 3);

comment on column public.conversation_cycle_commands.ai_inference_attempt_count is 'Provider-neutral count of locally authorized inference requests (maximum 3); it does not guarantee provider receipt or exactly-once delivery.';
...
create function public.reserve_customer_answer_ai_inference_attempt(target_command_id uuid,target_source_message_id uuid,expected_runtime_revision integer,expected_knowledge_version integer,execution_owner_id uuid) returns jsonb
...
create function public.defer_customer_message_ai_retry(target_command_id uuid,target_source_message_id uuid,execution_owner_id uuid,failure_code text) returns jsonb
language plpgsql security definer set search_path=public,pg_temp as $$
