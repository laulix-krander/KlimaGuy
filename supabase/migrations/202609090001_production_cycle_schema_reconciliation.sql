-- AP-16-06-06D-3: forward reconciliation after Production was observed with the
-- base command schema plus ai_inference_attempt_count, but without prerequisite
-- context, commit, and execution authority. Historical migrations are intentionally
-- neither rewritten nor blindly replayed; this migration reconciles current authority.
...
begin
 ...
 left join information_schema.columns c on c.table_schema='public' and c.table_name='conversation_cycle_commands' and c.column_name=e.name
 where c.column_name is null or c.data_type<>e.type_name or (e.must_not_be_null and c.is_nullable<>'NO');
