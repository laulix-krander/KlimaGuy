-- AP-16-06-06D-9: one bounded rehabilitation epoch for exhausted technical failures.
alter table public.conversation_cycle_commands
  add column if not exists technical_rehabilitation_count integer not null default 0;

alter table public.conversation_cycle_commands
  drop constraint if exists conversation_cycle_commands_technical_rehabilitation_count_check;
alter table public.conversation_cycle_commands
  add constraint conversation_cycle_commands_technical_rehabilitation_count_check
  check (technical_rehabilitation_count between 0 and 1);
...
declare
 ...
 rehabilitated boolean:=false; previous_attempts integer;
begin
 ...
 if cmd.id is null then
  ...
  if cmd.command_type<>'customer_answer' or cmd.result_code<>'persistence_failed' or cmd.conversation_id<>c.id or cmd.project_id<>r.project_id or cmd.pending_interaction_id<>p.id or cmd.prompt_message_id<>p.prompt_message_id or cmd.expected_conversation_revision<>c.revision or cmd.expected_runtime_revision<>r.revision or cmd.expected_knowledge_version<>k then return jsonb_build_object('success',false,'code','interaction_not_current'); end if;
  if cmd.execution_attempt_count<10 and cmd.technical_rehabilitation_count=0 then
   update public.conversation_cycle_commands set status='processing',failed_at=null where id=cmd.id returning * into cmd;
  elsif cmd.execution_attempt_count=10 and cmd.technical_rehabilitation_count=0 then
   previous_attempts:=cmd.execution_attempt_count; rehabilitated:=true;
   update public.conversation_cycle_commands set status='processing',failed_at=null,technical_rehabilitation_count=1 where id=cmd.id returning * into cmd;
  else return jsonb_build_object('success',false,'code','interaction_not_current'); end if;
 end if;
 return jsonb_build_object('success',true,'replay',false,'command_id',cmd.id,'conversation_id',c.id,'project_id',r.project_id,'pending_interaction_id',p.id,'expected_conversation_revision',c.revision,'expected_runtime_revision',r.revision,'expected_knowledge_version',k,'message_sequence',m.sequence,'prompt_sequence',prompt_sequence,'technical_rehabilitated',rehabilitated,'technical_rehabilitation_count',cmd.technical_rehabilitation_count,'previous_execution_attempt_count',previous_attempts,'technical_epoch_attempt_limit',10);
end $$;
...
language plpgsql security definer set search_path=public,pg_temp as $$
declare claimed jsonb; cmd public.conversation_cycle_commands%rowtype; now_at timestamptz:=statement_timestamp(); reclaimed boolean; epoch_attempts integer;
begin
 if auth.role() is distinct from 'service_role' then raise insufficient_privilege using message='customer_message_cycle_service_role_required'; end if;
 ...
 epoch_attempts:=cmd.execution_attempt_count-(cmd.technical_rehabilitation_count*10);
 if epoch_attempts<0 or epoch_attempts>=10 then return jsonb_build_object('success',false,'code','interaction_not_current','command_id',cmd.id); end if;
 ...
 return claimed||jsonb_build_object('acquire_kind',case when reclaimed then 'reclaimed' else 'acquired' end,'execution_owner_id',execution_owner,'execution_lease_expires_at',now_at+make_interval(secs=>lease_seconds),'execution_attempt_count',cmd.execution_attempt_count+1,'technical_recovery_epoch_attempt_count',epoch_attempts+1);
end $$;
...
begin
 ...
 return query select cmd.id,cmd.source_message_id,coalesce(cmd.execution_lease_expires_at,cmd.last_execution_started_at,cmd.created_at),
  (cmd.status='failed' and cmd.execution_attempt_count>=10) as requires_technical_rehabilitation
 ...
 where cmd.command_type='customer_answer' and cmd.source_message_id is not null and
  ((cmd.status='processing' and (cmd.execution_lease_expires_at<=statement_timestamp() or cmd.execution_lease_expires_at is null) and cmd.execution_attempt_count-(cmd.technical_rehabilitation_count*10)<10)
   or (cmd.status='failed' and cmd.result_code='persistence_failed' and cmd.technical_rehabilitation_count=0 and cmd.execution_attempt_count<=10))
  and exists(select 1 from public.conversation_message_text mt where mt.message_id=source.id)
  and exists(select 1 from public.conversation_message_text mt where mt.message_id=prompt.id and mt.body=s.outbound_text)
...
revoke all on function public.claim_customer_message_cycle(uuid),public.acquire_customer_message_cycle_execution(uuid,uuid,integer),public.discover_recoverable_conversation_cycles(integer) from public,anon,authenticated;
grant execute on function public.claim_customer_message_cycle(uuid),public.acquire_customer_message_cycle_execution(uuid,uuid,integer),public.discover_recoverable_conversation_cycles(integer) to service_role;
comment on column public.conversation_cycle_commands.technical_rehabilitation_count is 'Bounded count (0..1) of authority-controlled exhausted persistence-failure rehabilitation epochs.';
comment on function public.discover_recoverable_conversation_cycles(integer) is 'Side-effect-free service-only discovery of ordinary recovery and one eligible exhausted technical rehabilitation.';
