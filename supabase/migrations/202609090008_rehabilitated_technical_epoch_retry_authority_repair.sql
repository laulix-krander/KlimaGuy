-- AP-16-06-06D-12: admit ordinary retries throughout the one rehabilitated technical epoch.
...
begin
 ...
 if cmd.id is null then
  ...
  if epoch_attempts between 0 and 9 then
...
begin
 ...
 return query select cmd.id,cmd.source_message_id,coalesce(cmd.execution_lease_expires_at,cmd.last_execution_started_at,cmd.created_at),
  (cmd.status='failed' and cmd.technical_rehabilitation_count=0 and cmd.execution_attempt_count=10) as requires_technical_rehabilitation
 ...
 where cmd.command_type='customer_answer' and cmd.source_message_id is not null and
  ((cmd.status='processing' and (cmd.execution_lease_expires_at<=statement_timestamp() or cmd.execution_lease_expires_at is null) and cmd.execution_attempt_count-(cmd.technical_rehabilitation_count*10) between 0 and 9)
   or (cmd.status='failed' and cmd.result_code='persistence_failed' and
    ((cmd.execution_attempt_count-(cmd.technical_rehabilitation_count*10) between 0 and 9)
     or (cmd.technical_rehabilitation_count=0 and cmd.execution_attempt_count=10))))
...
comment on function public.discover_recoverable_conversation_cycles(integer) is 'Side-effect-free service-only discovery of retryable persistence failures with remaining derived technical epoch budget, including the one rehabilitation transition.';
