-- Roadmap PR 2: project-bound Customer identity and Qualification V2 policy.
-- The old commit signature is removed to prevent PostgREST overload ambiguity.
drop function public.commit_mvp_ai_turn(uuid,jsonb,text,text,boolean,text);
...
declare c public.conversations%rowtype; m public.conversation_messages%rowtype; b public.conversation_transport_bindings%rowtype;
 turn_row public.mvp_ai_turns%rowtype; customer_row public.customers%rowtype; project_title text; inbound_text text; transcript jsonb; media_context jsonb;
begin
 ...
 return jsonb_build_object('status','acquired','turn_id',turn_row.id,'turn',jsonb_build_object('inbound_message_id',m.id,'conversation_id',c.id,
  ...
  'project',jsonb_build_object('title',project_title),'customer',jsonb_build_object('name_known',nullif(btrim(customer_row.first_name),'') is not null or nullif(btrim(customer_row.last_name),'') is not null,'first_name',nullif(btrim(customer_row.first_name),''),'last_name',nullif(btrim(customer_row.last_name),'')),'inbound',jsonb_build_object('message_id',m.id,'text',inbound_text),'transcript',transcript,'ready_media',media_context);
end$$;


create function public.commit_mvp_ai_turn(target_turn_id uuid, target_facts_patch jsonb,
 target_customer_name_patch jsonb, target_reply_text text, target_qualification_status text, target_needs_human boolean, target_human_reason text)
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
...
comment on function public.commit_mvp_ai_turn(uuid,jsonb,jsonb,text,text,boolean,text) is
 'Atomically commits an MVP turn and deterministic Project review handoff. Never creates, prices, approves, or sends an offer.';
revoke all on function public.commit_mvp_ai_turn(uuid,jsonb,jsonb,text,text,boolean,text) from public,anon,authenticated;
grant execute on function public.commit_mvp_ai_turn(uuid,jsonb,jsonb,text,text,boolean,text) to service_role;
