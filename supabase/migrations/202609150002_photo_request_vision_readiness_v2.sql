-- Roadmap PR 3: Photo Request + Vision Readiness V2. Forward-only authority update.
alter table public.project_media drop constraint project_media_category_check;
alter table public.project_media add constraint project_media_category_check check (category in ('room_overview','indoor_area','outdoor_area','indoor_unit_location','outdoor_unit_location','pipe_route','electrical_connection','condensate_route','facade','roof','balcony','floor_plan','technical_document','customer_document','other'));

create table public.project_media_ai_observations (
 id uuid primary key default gen_random_uuid(), project_id uuid not null, project_media_id uuid not null, turn_id uuid not null references public.mvp_ai_turns(id) on delete restrict,
 observation text not null check(char_length(btrim(observation)) between 1 and 240), review_status text not null default 'unreviewed' check(review_status in ('unreviewed','confirmed','rejected')),
 reviewed_by uuid references auth.users(id) on delete restrict, reviewed_at timestamptz, created_at timestamptz not null default statement_timestamp(), updated_at timestamptz not null default statement_timestamp(),
 foreign key(project_id,project_media_id) references public.project_media(project_id,id) on delete restrict, unique(turn_id,project_media_id)
);
alter table public.project_media_ai_observations enable row level security;
create policy "photo observations scoped staff read" on public.project_media_ai_observations for select to authenticated using(auth.uid() is not null and public.current_app_role() in ('admin','reviewer') and exists(select 1 from public.projects p where p.id=project_id and p.deleted_at is null));
revoke all on public.project_media_ai_observations from public,anon,authenticated; grant select on public.project_media_ai_observations to authenticated;
create trigger project_media_ai_observations_updated before update on public.project_media_ai_observations for each row execute function public.set_updated_at();

-- Remove the PR 2 overload before installing the exact PR 3 signature.
drop function public.commit_mvp_ai_turn(uuid,jsonb,jsonb,text,text,boolean,text);
...
declare c public.conversations%rowtype; m public.conversation_messages%rowtype; b public.conversation_transport_bindings%rowtype;
 turn_row public.mvp_ai_turns%rowtype; customer_row public.customers%rowtype; project_title text; inbound_text text; transcript jsonb; media_context jsonb; photo_coverage jsonb;
begin
 ...
 select title into project_title from public.projects where id=c.current_project_id and customer_id=c.customer_id and deleted_at is null;
 if project_title is null then return jsonb_build_object('status','not_applicable'); end if;
 select * into customer_row from public.customers where id=c.customer_id and deleted_at is null for share;
 if not found then return jsonb_build_object('status','not_applicable'); end if;
 ...
 select coalesce(jsonb_agg(jsonb_build_object('media_id',pm.id,'category',case when pm.category in ('room_overview','indoor_unit_location','outdoor_unit_location','pipe_route','electrical_connection','condensate_route') then pm.category else 'other' end,
 ...
 select coalesce(jsonb_agg(jsonb_build_object('category',category,'count',amount)),'[]'::jsonb) into photo_coverage from (select category,count(*)::integer amount from public.project_media where project_id=c.current_project_id and upload_status='ready' and deleted_at is null and media_type='image' and mime_type in ('image/jpeg','image/png','image/webp') and category in ('room_overview','indoor_unit_location','outdoor_unit_location','pipe_route','electrical_connection','condensate_route') group by category) coverage;
 return jsonb_build_object('status','acquired','turn_id',turn_row.id,'turn',jsonb_build_object('inbound_message_id',m.id,'conversation_id',c.id,
  ...
  'project',jsonb_build_object('title',project_title),'customer',jsonb_build_object('name_known',nullif(btrim(customer_row.first_name),'') is not null or nullif(btrim(customer_row.last_name),'') is not null,'first_name',nullif(btrim(customer_row.first_name),''),'last_name',nullif(btrim(customer_row.last_name),'')),'inbound',jsonb_build_object('message_id',m.id,'text',inbound_text),'transcript',transcript,'ready_media',media_context,'project_photo_coverage',photo_coverage);
end$$;
...
declare turn_row public.mvp_ai_turns%rowtype; c public.conversations%rowtype;
 ...
 missing_keys text[]:=array[]::text[]; missing_photos text[]:=array[]::text[]; requires_site_check boolean:=false; classification jsonb; resulting_qualification text;
begin
 ...
 -- Informational projection only: deterministic server policy, never a model-authored authority.
 insert into public.mvp_project_facts(project_id,fact_key,fact_value)
 select turn_row.project_id,'required_photo_categories',to_jsonb(array['room_overview','indoor_unit_location','outdoor_unit_location']::text[]
