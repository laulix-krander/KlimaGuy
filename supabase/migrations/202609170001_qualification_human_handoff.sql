-- Persist a real Project human-review handoff and expose its minimal state to later AI turns.
...
declare c public.conversations%rowtype; m public.conversation_messages%rowtype; b public.conversation_transport_bindings%rowtype;
 turn_row public.mvp_ai_turns%rowtype; customer_row public.customers%rowtype; project_row public.projects%rowtype; project_title text; inbound_text text; transcript jsonb; media_context jsonb; photo_coverage jsonb;
begin
 ...
 select * into project_row from public.projects where id=c.current_project_id and customer_id=c.customer_id and deleted_at is null;
 project_title:=project_row.title;
 if project_row.id is null then return jsonb_build_object('status','not_applicable'); end if;
...
begin
 ...
 for classification in select value from jsonb_array_elements(coalesce(target_media_classifications,'[]'::jsonb)) loop
   if jsonb_typeof(classification)<>'object' or classification-array['media_id','category','observation']::text[]<>'{}'::jsonb
      ...
      or classification->>'category' is null
      or classification->>'category' not in ('room_overview','indoor_unit_location','outdoor_unit_location','pipe_route','electrical_connection','condensate_route','other') then raise exception 'invalid_media_classification'; end if;
   ...
   update public.project_media set category=classification->>'category' where id=(classification->>'media_id')::uuid;
 ...
 -- Human escalation wins over readiness, but can only advance the pre-offer qualification lifecycle.
 if (requires_site_check or (target_qualification_status='needs_human' and target_needs_human))
    and p.status in ('new','collecting_information') then
   update public.projects set status='human_review',requires_human_review=true where id=p.id and status in ('new','collecting_information');
   p.status:='human_review';
 elsif p.status='new' then
   update public.projects set status='collecting_information' where id=p.id and status='new'; p.status:='collecting_information';
