...
describe("KlimaGuy agent settings domain and service", () => {
  ...
  it("returns explicit unpersisted defaults for no row and parses DB rows", async () => {
    ...
    expect(klimaguyAgentSettingsRowSchema.parse(row).revision).toBe(2);
    await expect(loadEffectiveKlimaGuySettings({ read: async () => row })).resolves.toMatchObject({ persisted: true, revision: 2, settings: DEFAULT_KLIMAGUY_AGENT_SETTINGS });
  });
  it("falls back safely when the optional runtime read fails", async () => {
  ...
  it("maps every field and expected revision to the narrow CAS RPC", async () => {
    ...
    const result = await updateKlimaGuySettings({ update: async (value) => { args = value; return { status: "updated", revision: 1, updated_at: "2026-09-18T11:00:00.000Z" }; } }, DEFAULT_KLIMAGUY_AGENT_SETTINGS, 0);
    expect(result.revision).toBe(1); expect(args.target_expected_revision).toBe(0); expect(args.target_tone).toBe("friendly"); expect(Object.keys(args)).toHaveLength(15);
  });
});
