...
const photos = ["room_overview", "indoor_unit_location", "outdoor_unit_location"].map((category) => ({ project_id: PROJECT, category, media_type: "image", mime_type: "image/jpeg", upload_status: "ready", deleted_at: null }));
...
describe("deterministische MVP-Angebotsbereitschaft", () => {
  ...
  it("never treats a required site check as offer-ready", () => {
    expect(evaluateMvpQualificationReadiness(complete.map((fact) => fact.key === "electrical_supply" ? { key: "electrical_supply", value: "requires_site_check" } : fact), PROJECT, photos)).toMatchObject({ ready: false, requiresSiteCheck: true });
  });
