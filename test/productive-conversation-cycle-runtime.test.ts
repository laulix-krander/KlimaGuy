...
const executionTrace:CustomerAnswerCycleExecutionTrace={answer_context_lookup_attempted:true,answer_context_reused:true,answer_context_bootstrap_attempted:true,answer_context_bootstrap_succeeded:true,answer_context_source:"direct_current_binding",interpreter_present:true,normalization_reached:true,normalization_succeeded:true,ai_eligibility_evaluated:true,ai_eligible:true,ai_result_lookup_attempted:true,ai_result_reused:false,ai_result_persist_attempted:true,ai_result_persist_succeeded:true,ai_reservation_attempted:true,ai_reservation_succeeded:true,ai_reservation_result_code:"reserved",ai_reservation_attempt_number:2,ai_interpreter_invoked:true,ai_interpreter_succeeded:true,ai_interpreter_outcome:"matched",ai_interpreter_failure_class:null,deterministic_cycle_branch:"with_claim",deterministic_cycle_invoked:true,deterministic_cycle_succeeded:false,deterministic_cycle_failure_code:"planner_failed",interpretation_failure_code:null,failure_persistence_attempted:true,failure_persistence_succeeded:true};
...
describe("AP-16-06-03 productive recovery", () => {
  ...
  it("logs one privacy-safe rehabilitation event and counts the atomic acquisition result", async () => {
    vi.mocked(discoverRecoverableConversationCycles).mockResolvedValueOnce(successfulDiscovery([command(1)]));
    vi.mocked(runPersistentCustomerMessageCycle).mockResolvedValueOnce({kind:"completed",technical_rehabilitation:{rehabilitation_count:1,previous_execution_attempt_count:10,fresh_epoch_budget:10}});
    const logger={info:vi.fn(),error:vi.fn()};
    ...
    expect(await response.json()).toMatchObject({technical_rehabilitated:1});
    expect(logger.info).toHaveBeenCalledWith({event:"conversation_cycle_technical_failure_rehabilitated",rehabilitation_count:1,previous_total_execution_attempt_count:10,fresh_epoch_budget:10,reason_code:"exhausted_persistence_failure_rehabilitated"});
