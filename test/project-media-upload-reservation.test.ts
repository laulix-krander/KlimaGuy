...
function source(options: { user?: boolean; role?: string | null; project?: boolean; error?: unknown } = {}) {
  ...
  const dataSource: ReservationDataSource = {
    ...
    insertProjectMedia: async (payload) => { calls.payload = payload; return { data: options.error ? null : { id: mediaId }, error: options.error ?? null }; },
  };
  return { dataSource, calls };
}
const generators = { uuid: (() => { const ids = [mediaId, filenameId]; return () => ids.shift() ?? crypto.randomUUID(); })(), now: () => "2026-07-27T12:00:00.000Z" };
